...
#### Interaction with `optimized-streaming` (quick path)
...
* `--plot-all` forces **every** locus through full genotyping (the quick path is
  skipped entirely).
* With catalog `PlotReadVisualization` conditions, the quick path still genotypes
  ...
  not fire keep their quick-path genotype (`"QuickGenotype": true`).
* `--disable-all-plots` suppresses all image generation, so the quick path is used
  wherever it applies.
