...
- **New analysis modes**:
  ...
  - `--analysis-mode optimized-streaming` significantly speeds up analysis of large catalogs (> ~10k loci) by using simple heuristics to detect which loci are almost certainly homozygous reference, and avoids running the full computationally-expensive genotyping algorithm on them. Its memory usage is also low (< 10Gb) and independent of catalog size, similar to `low-mem-streaming` mode.
