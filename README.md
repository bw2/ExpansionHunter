...
- **New analysis modes**:
  ...
  - `June 21, 2026`: these two new analysis modes now 1) fully support multi-threaded processing via `--threads N` and 2) perform reservoir sampling to reduce processing time at the small subset of loci with extremely high read pile-ups (see `--max-depth`).
